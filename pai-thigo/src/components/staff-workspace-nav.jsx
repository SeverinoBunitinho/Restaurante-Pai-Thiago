import { ArrowRight, Compass, Menu, Route, ShieldCheck } from "lucide-react";
  ...
  return (
    <>
      <div className="xl:hidden">
        <details className="luxury-card rounded-[1.5rem] p-4 [&_summary::-webkit-details-marker]:hidden">
          <summary className="flex list-none items-center justify-between gap-3">
            <span className="inline-flex items-center gap-2 text-sm font-semibold text-[var(--forest)]">
              <span className="inline-flex h-8 w-8 items-center justify-center rounded-full border border-[rgba(182,135,66,0.16)] bg-[rgba(255,255,255,0.7)] text-[var(--gold)]">
                <Menu size={16} />
              </span>
              Menu da operacao
            </span>
            <span className="rounded-full border border-[rgba(20,35,29,0.12)] bg-[rgba(255,255,255,0.74)] px-3 py-1 text-[11px] font-semibold uppercase tracking-[0.16em] text-[var(--sage)]">
          ...
          <nav className="mt-4 grid gap-2">
            {navigationItems.map((item) => (
              <ActiveLink
                ...
                href={item.href}
                exact={item.exact}
                className="rounded-[1rem] border border-[rgba(20,35,29,0.08)] bg-[rgba(255,255,255,0.74)] px-3 py-2.5 text-sm font-semibold text-[var(--forest)]"
                activeClassName="border-[rgba(182,135,66,0.24)] bg-[rgba(182,135,66,0.1)]"
              >
                <span className="inline-flex items-center gap-2">
                  <item.icon size={15} className="text-[var(--gold)]" />
                  {item.title}
                </span>
              </ActiveLink>
            ))}
      ...
      <aside className="staff-side-stack hidden xl:grid">
